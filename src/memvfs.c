...
sqlite3_vfs *sqlite3_memvfs(void) {
  return &mem_vfs; 
}
...
int sqlite3_memvfs_init(
  ...
  int rc = SQLITE_OK;
  char str0[18] = "before bug";
  ic0_debug_print((uint32_t)str0, 18);
  SQLITE_EXTENSION_INIT2(pApi);
  mem_vfs.pAppData = sqlite3_vfs_find(0);
  mem_vfs.szOsFile = sizeof(MemFile);
  char str[18] = "before register";
  ic0_debug_print((uint32_t)str, 18);
  rc = sqlite3_vfs_register(&mem_vfs, 1);
  char str1[18] = "after register";
  ic0_debug_print((uint32_t)str1, 18);
  /*
#ifdef MEMVFS_TEST
  if( rc==SQLITE_OK ){
